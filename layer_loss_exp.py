...
class Trainer:
    ...
    def update_train_data(self, model, itr):
        ...
        if self.method == 'flexmatch':
            Flexmatch(self.graph, prediction, self.node_threshold).select()
    ...
    def cal_loss(self, model_name, model, graph, criterion, record):
        ...
        if model_name=='ourModel' or model_name=='ourModel_basis':
            ...
            #     pass 
            # else:
            record[3].append(out.detach().cpu())
            if self.args.soft_flag:
    ...
    def train(self, wandb_record=False):
        ...
        main_loss_weight_list = [args.main_loss_weight]
        for weight in main_loss_weight_list:
            args.main_loss_weight=weight 
            ...
            self.data_save_dic = {
                ...
                "edge_label_list": edge_label_list,
                "state_record_list": state_record_list,
                'best_test_metric_list': best_test_metric_list,
                'out_list': out_list,
            ...
            while torch.sum(self.graph.node_pseudolabel>=0) <= torch.sum(self.graph['unlabeled_index']) and iteration_count<=args.max_iteration:
                ...
                # calculate the loss
                optimizer.zero_grad()
                out, loss = self.cal_loss(model_name=self.model_name, model=model_iter, graph=graph_iter, criterion=criterion, record=[loss_list, pseudo_loss_list, muti_layer_loss_list, out_list])
                ...
                if self.stopper.early_stop(epoch, val_metric):
                    ...
                    best_test_metric_list.append(best_test_metric_iter)
                    best_val_metric_iter = -torch.inf
            ...
            save_path = f'record/dynamic_logits/data/'
...
if __name__ == "__main__":
    ...
    parser.add_argument('--main_loss_weight', type=int, default=0.8)
